...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      ...
      transfer.name,
      to_sighthill.num,
      to_sighthill.company
    FROM 
      routes start_routes
    JOIN 
      routes to_transfer ON (start_routes.company = to_transfer.company AND start_routes.num = to_transfer.num)
    JOIN
      stops transfer ON transfer.id = to_transfer.stop_id
    JOIN 
      routes from_transfer ON transfer.id = from_transfer.stop_id
    JOIN 
      routes to_sighthill ON (from_transfer.company = to_sighthill.company AND from_transfer.num = to_sighthill.num)
    WHERE
